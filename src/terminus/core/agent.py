...
from terminus.shared.constants import DEFAULT_MODEL
...
def _get_enhanced_system_prompt() -> str:
    ...
    # For local models (Qwen, Ollama, Mistral), use the enhanced prompt that emphasizes tool usage
    if session.current_model and ("qwen" in session.current_model.lower() or 
                                  "ollama" in session.current_model.lower() or 
                                  "mistral" in session.current_model.lower()):
        try:
...
async def _try_force_tool_usage(message: str) -> Optional[str]:
    ...
    from terminus.tools.filesystem.directory import get_current_directory, change_directory
